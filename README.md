...
**v26.27 — Clearer P/L vs premium column labels (Wheel + Derivatives tabs)** (2026-08-23)
- **Open wheel campaigns stop calling premium income "P/L".** On an open campaign the realized figure is premiums + dividends only (the equity result is still unrealised), so the open summary table (and its CSV) now labels it **Income**, while closed campaigns — where the figure genuinely includes the equity gain/loss — read **Realized P/L**. The open card's last chip is now always **Net (MTM)**, showing **—** when Live prices are off rather than repeating the premium figure under a "P/L" header (the Premiums Banked chip already shows it).
- **The Derivatives tab now speaks one language.** The by-Type, by-Strategy, and Performance-by-Ticker tables all use **Net P/L** (options-only realized result, net of buybacks, can be negative) instead of "P/L". Performance by Ticker also renames **Total Net Prem → Premium Collected** (gross opening credit) and **P/L per DIT → Net P/L / DIT**, and reorders its columns into the premium-selling flow — Premium Collected → Premium Capture → Net P/L → efficiency — so the gross-collected and net-kept figures sit together and can't be mistaken for each other. The by-Strategy table's column order now matches by-Type.
- Display-only — no figure changed. Two inline hardcoded P/L colours were swapped for the shared `color_pnl_cell` helper (COLOURS single-source). Test suite at **477 tests**.
