...
Expected: `477 tests | 477 passed | 0 failed` (33 sections, with section 0 = end-to-end smoke check)
...
**effective_basis()** — `use_lifetime=True` returns raw `blended_basis` (no premium offset). Default returns `(cost - premiums - dividends) / total_shares` where `cost` is `remaining_lot_cost` when set, else `total_cost`.

**Remaining-lot basis (display only — `remaining_lot_cost` / `remaining_lot_basis()`)** — carry-full-cost keeps the sold lot's cost on `total_cost`/`blended_basis` (deferring the partial sale's equity P/L to close), which is correct for P/L, MTM, capital, and the `open_campaign_equity` reconcile but *inflates the displayed per-share basis* after a below-basis partial sale (e.g. a covered-call call-away: SOFI showed $47.88 with 100 of 200 shares called away at $18). `build_campaigns()` keeps a parallel FIFO lot deque (`lot_queue`, oldest-first; seeded with pool lot then entry, pushed on adds, popped on sells, rescaled on splits, reset on close) and stores the FIFO cost of shares **still held** in `Campaign.remaining_lot_cost` (equals `total_cost` when no partial sale; `None` on the lifetime path). `remaining_lot_basis(c)` = `remaining_lot_cost / total_shares` (gross), and `effective_basis()` nets premiums off it — both **display only**. **`blended_basis` / `total_cost` are never changed** — MTM (`compute_unrealized_pnl` ~:1409), `capital_deployed` (~:1587), tab3 cap-efficiency/`_camp_deployed`, and the reconcile all still use the carry-full values. Display sites routed to `remaining_lot_basis`: tab3 card + summary gross basis, `tab0` wheel strip, `report.py` `avg`, `position_snapshot` Entry **and** its Gross-Unreal mark (so gross-vs-net gap = real premium, not the carry-full inflation). Gross and net must move together on any surface showing both, or the "premium saved" delta goes nonsensical.

**Call-away labelling (`_find_call_away`)** — a share-sale row whose exact timestamp carries a `CALL` `Assignment` (Sub Type) row was called away, not sold at will. `build_campaigns()` appends the strike to the Exit event detail (`Sold N @ $x/sh (Called away — $18 Call)`); the event `type` stays `'Exit'` so it still passes the Share & Dividend Events filter (which drops types containing `assign`). `_find_call_away` filters `Call or Put == CALL` so put assignments (share *buys* on the entry side) never match.

**Net (MTM) — `campaign_net_mtm(c, last_price, use_lifetime)`** — pure/Streamlit-free helper for the Wheel tab's "Net (MTM)" column and the card's split P/L chip. Answers "what would realized P/L be if this OPEN campaign closed at `last_price` now" — the number the premiums-only `realized_pnl()` hides, because it folds the deferred call-away loss (still in carry-full `total_cost`) and the held-share mark back in. Non-lifetime: `exit_proceeds + premiums + dividends − total_cost + last×shares`. Lifetime: `last×shares − total_cost` (lifetime `total_cost` is already net of premiums/sales — don't add them again). Returns `None` for zero/negative price, a closed campaign, or no share position (caller shows "—"). `tab3` fetches live prices up-front (moved above the summary table) so the column, the Cap-Efficiency block, and the cards share one fetch; the `_campaign_net_mtm` closure just looks up the price and delegates. The column only renders when the 📡 Live toggle is on (dropped when all values are `None`). The card's live-strip **Unrealised** (held shares vs effective basis) and **Net (MTM)** (whole campaign incl. deferred loss) intentionally differ — position view vs campaign view.
...
- `test_tastymechanics.py` — 477 tests, 33 sections (section 0 = compute_app_data end-to-end smoke check; gates the rest)
